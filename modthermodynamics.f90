...
!! calculate the fields at the half levels, and finally calculate the virtual potential temperature.
  subroutine thermodynamics(simid)
   ...
   !cstep  integer:: Nair(k1)     !cstep IBM
    ...
    call diagfld    !cstep does not take into account the presence of buildings
    call calc_halflev !calculate halflevel values of qt0 and thl0

    if (lmoist .and. (.not. lnoclouds)) then
      call icethermoh !cstep has been adapted to exclude buildings
    end if
    ...
    if (.not.(lapply_ibm .and. simid == Nsim)) then
       ...
       call airslabsum(thvh,1,k1,thv0h,2-ih,i1+ih,2-jh,j1+jh,1,k1,2,i1,2,j1,1,k1)
       call airslabsum(thvf,1,k1,thv0,2-ih,i1+ih,2-jh,j1+jh,1,k1,2,i1,2,j1,1,k1)
    endif
...
!! \author Steef B\"oing
  ...
  !cstep IBM use modruraldata, only : bc_height !cstep IBM
  use modfields, only : qvsl,qvsi,qt0,thl0,exnf,presf,tmp0,ql0,esl,qsat,ksfc
  implicit none

  integer i, j, k
  real :: ilratio, esl1,esi1,qvsl1,qvsi1,qsatur, thlguess, thlguessmin,tlo,thi,ttry
  real :: Tnr,Tnr_old
  integer :: niter,nitert,tlonr,thinr !cstep IBM , kmin !cstep IBM

!     calculation of T with Newton-Raphson method
!     first guess is Tnr=tl
      nitert = 0
      niter = 0
      !cstep IBM kmin=bc_height(i+myidx*imax,j+myidy*jmax)+1 !cstep IBM
      !cstep IBM do k=kmin,k1                                !cstep IBM
      !cstep IBM do k=1,k1
      do j=2,j1
      do i=2,i1
      do k=ksfc(i,j),k1 
...
!! \author Steef B\"oing
  ...
  use modglobal, only : i1,j1,k1,rd,rv,rlv,tup,tdn,cp,ttab,esatltab,esatitab
  use modfields, only : qt0h,thl0h,exnh,presh,ql0h,ksfc
  !cstep IBM use modruraldata, only : bc_height  !cstep IBM
  implicit none
  ...
  real :: ilratio, esl1, esi1, qvsl1,qvsi1, qsatur, thlguess, thlguessmin,tlo,thi,ttry
  real :: Tnr,Tnr_old
  integer :: niter,nitert,tlonr,thinr,kmin !cstep IBM
...
!! If the kappa advection scheme is active, interpolation needs to be done consistently.
  subroutine calc_halflev
    ...
    use modfields, only : thl0,thl0h,qt0,qt0h,ksfc
    use modsurfdata,only: qts,thls
    use modibmdata, only:  thlwall  !cstep IBM bc_height
    implicit none
...
!cstep IBM take care if one want to use another qt surface boundary condition
!cstep IBM      do  k=2,k1
        do  j=2,j1
          do  i=2,i1
          !cstep IBM kmin=bc_height(i+myidx*imax,j+myidy*jmax)+1
           !cstep IBM do  k = max(2,kmin),k1
          do k = max(2,ksfc(i,j)),k1
...
!cstep IBM    if (iadv_qt==iadv_kappa) then
!cstep IBM        call halflev_kappa(qt0,qt0h)
!cstep IBM    else  
!cstep IBM      do  k=2,k1
!cstep IBM        do  j=2,j1
!cstep IBM          do  i=2,i1
!cstep IBM            qt0h(i,j,k)  = (qt0 (i,j,k)*dzf(k-1)+qt0 (i,j,k-1)*dzf(k))/(2*dzh(k))
!cstep IBM          end do
!cstep IBM        end do
!cstep IBM      end do
!cstep IBM      qt0h(2:i1,2:j1,1)  = qts
!cstep IBM    end if
