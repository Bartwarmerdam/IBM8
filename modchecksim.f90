...
!> Checks local and total divergence
  subroutine chkdiv
    ...
    use modglobal, only : i1,j1,kmax,dx,dy,dzf,dt_reason !cstep ,imax,jmax  !cstep IBM
    use modfields, only : u0,v0,w0,rhobf,rhobh, ksfc                !cstep IBM
    !cstep IBM use modruraldata, only : bc_height                              !cstep IBM
    use mpi
    use modmpi,    only : myid,comm3d,mpi_sum,mpi_max,my_real,mpierr !cstep ,myidx,myidy  !cstep IBM
    implicit none
    ...
    integer i, j, k !cstep ,   kmin     !cstep IBM
    ...
    do j=2,j1
    do i=2,i1
    !cstep kmin=bc_height(i+myidx*imax,j+myidy*jmax)+1  !cstep IBM
    !do k=kmin,kmax   !cstep IBM
...
!> MK: ckhwallvel: Checks average u velocity inside the walls !cstep IBM
  subroutine chkwallvel

    use modglobal, only : i1,ih,j1,jh,k1,cu
    use modfields, only : um,ksfc
    use modmpi,    only : myid,comm3d,mpi_sum,mpi_max,my_real,mpierr
    use modibmdata, only : lapply_ibm,wallslabsum, Nwall
    implicit none

    real, allocatable, dimension(:) :: uwav
!cstep    integer Nwall
    integer k,ksfc_max
    if (.not. (lapply_ibm)) return
 

    ksfc_max = maxval(ksfc)-1
    allocate (uwav(k1))
...
!cstep     uwallavg = 0.
    ...
    call wallslabsum(uwav  ,1,k1,abs(um)  ,2-ih,i1+ih,2-jh,j1+jh,1,k1,2,i1,2,j1,1,k1) !cstep,Nwall)
!cstep    uwallavg  = uwav  /Nwall + cu
    uwav(1:ksfc_max) = uwav(1:ksfc_max)/Nwall (1:ksfc_max)

    if(myid==0)then
      do k=1,ksfc_max
        write(6 ,*) 'u_wallavg = ', k,uwav(k)
      enddo
   end if
