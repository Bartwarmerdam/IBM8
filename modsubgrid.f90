...
!-----------------------------------------------------------------|
  ...
  use modmpi,      only : excjs !cstep IBM ,myidx,myidy !cstep IBM
  use modibmdata,  only : applydamping !cstep IBM ,bc_height !cstep IBM
  implicit none

  real    :: strain2,mlen
  integer :: i,j,k,kp,km,jp,jm  !cstep IBM ,kmin  !cstep IBM
  ...
  if(lsmagorinsky) then
    ...
    do i = 2,i1
      do j = 2,j1
        ...
        !cstep IBM   do k=kmin,kmax    !cstep IBM 
        do k=ksfc(i,j),kmax
          kp=k+1
          km=k-1
          jp=j+1
          ...
          !cstep IBM if(k == kmin) then   !cstep IBM
          if (k == ksfc(i,j)) then    !cstep IBM 
            strain2 =  ( &
          ...
          call applydamping(ekm(i,j,k),i,j,k)   ! cstep IBM < MK - apply damping function in modibmdata.f90 when it is used.
  ...
  !cstep IBM  do k=1,kmax
   do j=2,j1
     do i=2,i1
        !cstep IBM kmin=bc_height(i+myidx*imax,j+myidy*jmax)+1  !cstep IBM
        !cstep IBM do k=kmin,kmax                               !cstep IBM
         do k=ksfc(i,j),kmax  !cstep IBM
...
!-----------------------------------------------------------------|
  ...
  !cstep IBM use modmpi,         only: myidx,myidy  !cstep IBM

  implicit none

  real    tdef2, uwflux, vwflux, local_dudz, local_dvdz, local_dthvdz, horv
  integer i,j,k,jm,jp,km,kp, kmin !cstep IBM, maintain kmin as a shortcut


  !cstep IBM do k=2,kmax
  do j=2,j1
  do i=2,i1
!cstep IBM  kmin=bc_height(i+myidx*imax,j+myidy*jmax)+1 !cstep IBM
!cstep IBM  do k=(kmin+1),kmax  !cstep IBM
  do k=ksfc(i,j)+1,kmax   !cstep IBM
    kp=k+1
    km=k-1
    jp=j+1
    jm=j-1
...
!     --------------------------------------------
!     special treatment for lowest full level: k=kmin  !cstep IBM 1
!     --------------------------------------------
...
!            sbshr(2:i1,2:j1,1:kmax)+sbbuo(2:i1,2:j1,1:kmax)+sbdiss(2:i1,2:j1,1:kmax)
  ...
  subroutine diffc (putin,putout,flux)
    ...
    !cstep IBM use modmpi,    only :  myidx,myidy,myid !cstep IBM
    !cstep IBM use modruraldata, only : bc_height     !cstep IBM
    ...
    do j=2,j1
      ...
      do i=2,i1
        ...
        !cstep IBM do k=(kmin+1),kmax
        do k=ksfc(i,j) + 1, kmax
          kp=k+1
          km=k-1
        ...
        putout(i,j,kmin) = putout(i,j,kmin) &
                ...
                ( rhobh(kmin+1)/rhobf(kmin) * (dzf(kmin+1)*ekh(i,j,kmin) + dzf(kmin)*ekh(i,j,kmin+1)) &
                  *  (putin(i,j,kmin+1)-putin(i,j,kmin)) / dzh(kmin+1)**2 &
                  + rhobh(kmin)/rhobf(kmin)*flux(i,j) *2.                        )/dzf(1) &
                          )
  ...
  subroutine diffe(putout)
    ...
    !cstep IBM use modmpi,    only : myidx,myidy
    !cstep IBM use modruraldata, only : bc_height
    ...
    do j=2,j1
      ...
      do i=2,i1
        ...
        fu = ( 0.5*( ustar(i,j)+ustar(i-1,j) ) )**2  * &
                upcu/sqrt(upcu**2  + &
                ((v0(i,j,kmin)+v0(i-1,j,kmin)+v0(i,jp,1)+v0(i-1,jp,kmin))/4.+cv)**2)
  ...
  !*****************************************************************
    ...
    do j=2,j1
      ...
      jm=j-1
      do i=2,i1
        !cstep IBM !cstep IBM kmin=bc_height(i+myidx*imax,j+myidy*jmax)+1
        !cstep IBM do k=(kmin+1),kmax
        do k=ksfc(i,j)+1,kmax
          kp=k+1
          km=k-1
