...
!> Calculates the interaction with the soil, the surface temperature and humidity, and finally the surface fluxes.
  subroutine surface
    use modglobal,  only : i1,j1,fkar,zf,cu,cv,nsv,ijtot,rd,rv,imax,jmax,dzf !cstep IBM
    use modfields,  only : thl0, qt0, u0, v0, u0av, v0av,ksfc
    use mpi
    use modmpi,     only : my_real, mpierr, comm3d, mpi_sum, excjs, mpi_integer, myid, myidx, myidy !cstep IBM
    use moduser,    only : surf_user
    use modibmdata, only : bc_height !cstep IBM
    implicit none
    ...
    if(lhetero) then
      ...
      do j = 2, j1
        do i = 2, i1
          !k=bc_height(i+myidx*imax,j+myidy*jmax)  !cstep IBM
          patchx = patchxnr(i)
          patchy = patchynr(j)
          upatch(patchx,patchy) = upatch(patchx,patchy) + 0.5 * (u0(i,j,ksfc(i,j)) + u0(i+1,j,ksfc(i,j))) !cstep IBM
          vpatch(patchx,patchy) = vpatch(patchx,patchy) + 0.5 * (v0(i,j,ksfc(i,j)) + v0(i,j+1,ksfc(i,j))) !cstep IBM
    ...
    if(isurf <= 2) then
      ...
      do j = 2, j1
        do i = 2, i1
          kmin=ksfc(i,j)!k=bc_height(i+myidx*imax,j+myidy*jmax)  !cstep IBM
          if(lhetero) then
            patchx = patchxnr(i)
            patchy = patchynr(j)
          endif
          ...
          !cstep IBM k=1 replaced by k+1
          Cm(i,j) = fkar ** 2. / (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / obl(i,j)) + psim(z0m(i,j) / obl(i,j))) ** 2.
          Cs(i,j) = fkar ** 2. / (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / obl(i,j)) + psim(z0m(i,j) / obl(i,j))) / &
          (log(0.5*dzf(kmin) / z0h(i,j)) - psih(0.5*dzf(kmin) / obl(i,j)) + psih(z0h(i,j) / obl(i,j)))
          ...
          if(lmostlocal) then
            ...
            if (lhetero) then
              ...
              horvav  = sqrt(u0av(kmin) ** 2. + v0av(kmin) ** 2.)
              horvav  = max(horvav, 0.1)
              ra(i,j) = 1. / ( Cs(i,j) * horvav )  !cstep IBM , this needs to be refined
            endif
          end if
    ...
    if(isurf <= 2) then
      do j = 2, j1
        do i = 2, i1
          kmin=ksfc(i,j)  !cstep IBM
          upcu   = 0.5 * (u0(i,j,kmin) + u0(i+1,j,kmin)) + cu !cstep IBM
          vpcv   = 0.5 * (v0(i,j,kmin) + v0(i,j+1,kmin)) + cv !cstep IBM
          horv   = sqrt(upcu ** 2. + vpcv ** 2.)
          horv   = max(horv, 0.1)
          horvav = sqrt(u0av(kmin) ** 2. + v0av(kmin) ** 2.) !cstep IBM, take care, slab mean goes through buildings
                   !cstep better exclude this BC if using IBM
          horvav = max(horvav, 0.1)
          ...
          phimzf = phim(0.5*dzf(kmin)/obl(i,j))
          phihzf = phih(0.5*dzf(kmin)/obl(i,j))
          
          dudz  (i,j) = ustar(i,j) * phimzf / (fkar*0.5*dzf(kmin))*(upcu/horv)
          dvdz  (i,j) = ustar(i,j) * phimzf / (fkar*0.5*dzf(kmin))*(vpcv/horv)
          dthldz(i,j) = - thlflux(i,j) / ustar(i,j) * phihzf / (fkar*0.5*dzf(kmin))
          dqtdz (i,j) = - qtflux(i,j)  / ustar(i,j) * phihzf / (fkar*0.5*dzf(kmin))
        end do
      end do
      ...
      if(lsmoothflux) then
        ...
        do j = 2, j1
          do i = 2, i1
            ...
            dudz  (i,j) = ustar(i,j) * phimzf / (fkar*0.5*dzf(kmin))*(upcu/horv) !cstep IBM 
            dvdz  (i,j) = ustar(i,j) * phimzf / (fkar*0.5*dzf(kmin))*(vpcv/horv) !cstep IBM
            dthldz(i,j) = - thlflux(i,j) / ustar(i,j) * phihzf / (fkar*0.5*dzf(kmin)) !cstep IBM
            dqtdz (i,j) = - qtflux(i,j)  / ustar(i,j) * phihzf / (fkar*0.5*dzf(kmin)) !cstep IBM
          end do
        end do
      ...
      do j = 2, j1
        do i = 2, i1
          ...
          horv   = sqrt(upcu ** 2. + vpcv ** 2.)
          horv   = max(horv, 0.1)
          horvav = sqrt(u0av(kmin) ** 2. + v0av(kmin) ** 2.) !cstep IBM
          horvav = max(horvav, 0.1)
          if( isurf == 4) then !cstep IBM
            if(lmostlocal) then
              ustar (i,j) = fkar * horv  / (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / obl(i,j)) + psim(z0m(i,j) / obl(i,j)))
            else
              if(lhetero) then
                ustar (i,j) = fkar * horvpatch(patchx,patchy) / (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / obl(i,j))&
                + psim(z0m(i,j) / obl(i,j)))
              else
                ustar (i,j) = fkar * horvav / (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / obl(i,j)) + psim(z0m(i,j) / obl(i,j)))
              endif
            end if
          else
            if(lhetero) then
          ...
          phimzf = phim(0.5*dzf(kmin)/obl(i,j)) !cstep IBM
          phihzf = phih(0.5*dzf(kmin)/obl(i,j)) !cstep IBM
          ...
          tskin(i,j) = min(max(thlflux(i,j) / (Cs(i,j) * horv),-10.),10.)  + thl0(i,j,kmin) !cstep IBM
          qskin(i,j) = min(max( qtflux(i,j) / (Cs(i,j) * horv),-5e-2),5e-2) + qt0(i,j,kmin) !cstep IBM
      ...
      call MPI_ALLREDUCE(thlsl, thls, 1,  MY_REAL, MPI_SUM, comm3d,mpierr) !cstep check whether this makes sense
    ...
    call excjs(ustar,2,i1,2,j1,1,1,1,1)  !cstep this differs from the excj command in the previous version
      ...
      do j = 2, j1
        do i = 2, i1
         ! k=bc_height(i+myidx*imax,j+myidy*jmax)
          ...
          qskin(i,j) = surfwet * qsatsurf + (1. - surfwet) * qt0(i,j,ksfc(i,j))
          qtsl       = qtsl + qskin(i,j)
        end do
      end do
...
!> Calculates the Obukhov length iteratively.
  subroutine getobl !cstep IBM
    use modglobal, only : zf, rv, rd, grav, i1, j1, i2, j2, cu, cv, imax, jmax, dzf
    use modfields, only : thl0av, qt0av, u0, v0, thl0, qt0, u0av, v0av, ksfc
    use mpi
    use modmpi,    only : my_real,mpierr,comm3d,mpi_sum,mpi_integer,myidx,myidy
    !use modibmdata, only : bc_height
    implicit none

    integer             :: i,j,iter,patchx,patchy,kmin
      ...
      do i=2,i1
        do j=2,j1
          ...
          if(lhetero) then
            patchx = patchxnr(i)
            patchy = patchynr(j)
            Rib    = grav / thvs_patch(patchx,patchy) * 0.5*dzf(kmin) * (thv - thvsl) / horv2
          else
            Rib    = grav / thvs * 0.5*dzf(kmin) * (thv - thvsl) / horv2
          endif
          ...
          if (Rib == 0) then
             ...
             do while (.true.)
                iter    = iter + 1
                Lold    = L
                fx      = Rib - 0.5*dzf(kmin) / L * (log(0.5*dzf(kmin) / z0h(i,j)) - psih(0.5*dzf(kmin) / L) + psih(z0h(i,j) / L)) /&
                     (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / L) + psim(z0m(i,j) / L)) ** 2.
                Lstart  = L - 0.001*L
                Lend    = L + 0.001*L
                fxdif   = ( (- 0.5*dzf(kmin) / Lstart * (log(0.5*dzf(kmin) / z0h(i,j)) - psih(0.5*dzf(kmin) / Lstart) + psih(z0h(i,j) / Lstart)) /&
                     (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / Lstart) + psim(z0m(i,j) / Lstart)) ** 2.) - (-0.5*dzf(kmin) / Lend * &
                     (log(0.5*dzf(kmin) / z0h(i,j)) - psih(0.5*dzf(kmin) / Lend) + psih(z0h(i,j) / Lend)) / (log(0.5*dzf(kmin) / z0m(i,j)) - psim(0.5*dzf(kmin) / Lend)&
    ...
    if(lhetero) then
      ...
      do j = 2, j1
        do i = 2, i1
          kmin=ksfc(i,j)!k=bc_height(i+myidx*imax,j+myidy*jmax)
          patchx = patchxnr(i)
          patchy = patchynr(j)
          upatch(patchx,patchy)    = upatch(patchx,patchy)    + 0.5 * (u0(i,j,kmin) + u0(i+1,j,kmin))
          vpatch(patchx,patchy)    = vpatch(patchx,patchy)    + 0.5 * (v0(i,j,kmin) + v0(i,j+1,kmin))
          Npatch(patchx,patchy)    = Npatch(patchx,patchy)    + 1
          lthlpatch(patchx,patchy) = lthlpatch(patchx,patchy) + thl0(i,j,kmin)
          lqpatch(patchx,patchy)   = lqpatch(patchx,patchy)   + qt0(i,j,kmin)
      ...
      do patchy = 1, ypatches
        do patchx = 1, xpatches
          ...
          do while (.true.) !no IBM correction? Is that right? MK does so as well ,Bart
            iter    = iter + 1
            Lold    = L
...
!> Calculates surface resistance, temperature and moisture using the Land Surface Model
  subroutine do_lsm !cstep not adapted for IBM
