...
!----------------------------------------------------------------
  ...
  use modibm,   only : applyibm, exitibm, zerowallvelocity ! cstep IBM MK
  use modibmdata,    only : lpoislast !cstep IBM MK


!cstep CPM_1 the following modules are needed if the concurrent precursor method is applied
!cstep  use modwindturbinedata, only : initadditionalfields, calcadditionalfields, exitadditionalfields, initwindturbine, chkwindturbine, exitwindturbinedata, windcontrol !PVD
!cstep  use modwindturbine,     only : windturbineforces !PVD
  use modnudgeboundary, only : initnudgeboundary, nudgeboundary, loadfields, savefields, exitnudgeboundary, lnudgeboundary, Nsim, statid, turid, refid !PVD
  use modmpi, only : myid ! PVD
  use modfields, only : u0, v0, w0, thl0, qt0, e120 ! PVD



  implicit none
  integer simid   !cstep CPM_2 needed for concurrent precursor 
...
! call initchem
  ...
  call initnudgeboundary !cstep CPM_3
...
!------------------------------------------------------
  ...
  do while (timeleft>0) !step  .or. rk3step < 3)   , rk3step will now not be changed in tstep
  do simid = 1,Nsim ! cstep CPM_4a  1=reference, 2=obstacles like buildings, wind turbines or hills

    if (simid == refid) call tstep_update      ! Calculate all various time counters, cstep CPM_5
    if (lnudgeboundary) call loadfields(simid) ! cstep CPM_6
    do rk3step =1,3     !cstep CPM_10a
...
!JvdD    call tqaver !set thl, qt and sv(n) equal to slab average at level kmax
    ...
    !cstep IBM
    !< MK: Ordering of the Poisson Solver and the IBM, (lpoislast==.true.): IBM -> Pois, (lpoislast==.false.): zerowallvelocity -> Pois -> IBM
    !< MK: If lapply_ibm is not defined or set to .false., the Immersed boundary functions will not be applied
    if(lpoislast == .true.)  call applyibm(simid) !Apply ibm
    if(lpoislast == .false.) call zerowallvelocity(simid)   !Apply correction on the walls before poisson to reduce loss of mass due to removal of leaking
    call poisson                                     !Apply Poisson Solver
    if(lpoislast == .false.) call applyibm(simid) !Apply ibm


    call samptend(tend_pois,lastterm=.true.)
    if (lnudgeboundary .and. simid == turid) call nudgeboundary !verplaatst Bart 
...
!-----------------------------------------------------
    call thermodynamics(simid)
    call leibniztend
!-----------------------------------------------------
!   3.7  WRITE RESTARTFILES AND DO STATISTICS
!------------------------------------------------------
    if (simid == statid) then !cstep CPM_11a
    call twostep
    !call coldedge
    call checksim
    ...
    end if !cstep, ends simid == statid  ,  CPM_11b
    end do !cstep  , CPM_10b, end of rk3step

    if (lnudgeboundary) call savefields(simid) ! cstep , CPM_8
  
  end do   !cstep CPM_4b: close the loop over reference and obstacle simulations
...
!-------------------------------------------------------
  ...
  call exitcanopy
  call exitibm           !cstep IBM  MK
  call exitmodules

  call exitnudgeboundary !cstep, CPM_9
!cstep  call exitadditionalfields !cstep
!cstep  call exitwindturbinedata
